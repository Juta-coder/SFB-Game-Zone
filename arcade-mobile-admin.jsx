...
// ── Mock aggregate figures (kept consistent with desktop console) ──
const A_TODAY_PLAYS = { tap: 412, memory: 287, catch: 358, reaction: 195, whack: 248 };
const A_LIVE = { tap: 28, memory: 14, catch: 19, reaction: 9, whack: 17 };
const A_DEPTS = ['Production', 'R&D', 'Sales', 'HR', 'Finance', 'Logistics'];

// ── Build the workbook (3 sheets) from the app's data ──────────
function buildArcadeReport(rangeLabel) {
  const today = new Date();
  const stamp = today.toLocaleString('th-TH', { dateStyle: 'medium', timeStyle: 'short' });
  const totalPlays = Object.values(A_TODAY_PLAYS).reduce((a, b) => a + b, 0);
  const liveNow = Object.values(A_LIVE).reduce((a, b) => a + b, 0);

  // Sheet 1 — Overview
  const overview = [
    ['SFB Arcade — รายงานสรุป', ''],
    ['ช่วงข้อมูล', rangeLabel],
    ['สร้างเมื่อ', stamp],
    ['', ''],
    ['ตัวชี้วัด', 'ค่า'],
    ['ผู้เล่นออนไลน์ตอนนี้ (คน)', liveNow],
    ['จำนวนการเล่นวันนี้ (ครั้ง)', totalPlays],
    ['ผู้เล่นทั้งหมด (คน)', 284],
    ['การเล่นสะสม 7 วัน (ครั้ง)', 8742],
    ['', ''],
    ['เกม', 'เล่นวันนี้', 'ออนไลน์', 'พฤติกรรม SFB'],
    ...A_GAMES.map(g => [g.name, A_TODAY_PLAYS[g.id], A_LIVE[g.id], (A_PILLARS[g.pillar] || {}).name || '']),
  ];

  // Sheet 2 — Player scores (flatten leaderboards)
  const scores = [['employee_id', 'name', 'department', 'game', 'pillar', 'score', 'unit', 'recorded_at']];
  A_GAMES.forEach(g => {
    A_LB[g.id].forEach((r, i) => {
      const dept = A_DEPTS[(r.id.charCodeAt(3) + i) % A_DEPTS.length];
      const hh = String(8 + (i % 10)).padStart(2, '0');
      const mm = String((i * 13) % 60).padStart(2, '0');
      scores.push([
        r.id, r.name, dept, g.en, (A_PILLARS[g.pillar] || {}).name || '',
        r.score, r.unit, `2026-06-04 ${hh}:${mm}`,
      ]);
    });
  });

  // Sheet 3 — Peer recognition ("ใครคือที่สุด")
  const peer = [['พฤติกรรม', 'เสาหลัก', 'คำอธิบาย', 'ผู้ได้รับโหวตสูงสุด', 'รหัส', 'จำนวนโหวต']];
  A_PAIRS.forEach((q, qi) => {
    const offset = qi * 3;
    const winner = A_NAMES[offset % A_NAMES.length];
    const votes = 38 - (qi % 5) * 4;
    peer.push([
      q.concept, (A_PILLARS[q.pillar] || {}).name || '', q.desc,
      winner[0], winner[1], votes,
    ]);
  });

  return [
    { name: 'สรุปภาพรวม', rows: overview },
    { name: 'คะแนนผู้เล่น', rows: scores },
    { name: 'ใครคือที่สุด', rows: peer },
  ];
}
...
function buildReportFromLive(data, rangeLabel) {
  ...
  const overview = [
    ['SFB Arcade — รายงานสรุป (ข้อมูลจริงจาก Google Sheet)', ''],
    ['ช่วงข้อมูล', rangeLabel],
    ['สร้างเมื่อ', stamp],
    ['', ''],
    ['ตัวชี้วัด', 'ค่า'],
...
function AdminPanel({ onLogout }) {
  ...
  const liveNow = Object.values(A_LIVE).reduce((a, b) => a + b, 0);
  const playsToday = Object.values(A_TODAY_PLAYS).reduce((a, b) => a + b, 0);
  ...
  const generate = async () => {
    ...
    let live = null;
    if (connected) {
      try { live = await window.ArcadeAPI.fetchReport(range); } catch (e) { console.warn(e); }
    }
    await new Promise(r => setTimeout(r, 350)); // perceptible delay
    try {
      const sheets = live ? buildReportFromLive(live, rangeLabel) : buildArcadeReport(rangeLabel);
      const blob = window.buildXLSX(sheets);
      const tag = live ? 'live' : 'demo';
      const fname = `SFB_Arcade_report_${range}_${tag}_2026-06-04.xlsx`;
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: A_BRAND.paper, overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)' }}>SFB Arcade · ผู้ดูแล</div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 22px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <AdminStat label="ออนไลน์ตอนนี้" value={A_fmt.num(liveNow)} sub="คน" color={A_BRAND.teal} live />
          <AdminStat label="เล่นวันนี้" value={A_fmt.num(playsToday)} sub="ครั้ง" color={A_BRAND.navy} />
          <AdminStat label="ผู้เล่นทั้งหมด" value={A_fmt.num(284)} sub="คน" color={A_BRAND.red} />
          <AdminStat label="ตอบ ‘ใครคือที่สุด’" value={A_fmt.num(18)} sub="คน" color="#d59a16" />
        </div>
            ...
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: A_BRAND.tealDeep }}>
                  ดาวน์โหลดแล้ว{source === 'live' ? ' · ข้อมูลจริง' : source === 'demo' ? ' · ข้อมูลตัวอย่าง' : ''}
                </div>
...
Object.assign(window, { MobileAdmin, buildArcadeReport });

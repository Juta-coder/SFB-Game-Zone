...
function LoginScreen({ onLogin, onAdmin }) {
  ...
  const ready = name.trim() && empId.trim();
  const submit = () => {
    if (!ready) return;
    onLogin({ name: name.trim(), empId: empId.trim().toUpperCase() });
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 8 }}>
        ...
        <div style={{ marginTop: 16, fontSize: 26, fontWeight: 800, color: BRAND.navy, letterSpacing: -0.3 }}>SFB Game Zone</div>
      ...
      <div style={{
        ...
        <label style={{ fontSize: 13, color: BRAND.inkSoft, fontWeight: 600 }}>ชื่อ-นามสกุล</label>
        <input value={name} onChange={e => setName(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter') submit(); }}
          placeholder="พิมพ์ชื่อของคุณ" autoFocus
          style={{
            width: '100%', marginTop: 8, padding: '14px 16px', borderRadius: 14,
            border: `1.5px solid ${BRAND.hairline}`, fontSize: 18, fontWeight: 600,
            color: BRAND.ink, fontFamily: 'Prompt', outline: 'none', background: BRAND.paper,
          }} />

        <label style={{ display: 'block', marginTop: 16, fontSize: 13, color: BRAND.inkSoft, fontWeight: 600 }}>
          รหัสพนักงาน
        </label>
        <input value={empId} onChange={e => setEmpId(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter') submit(); }}
          placeholder="เช่น TW-1042"
          style={{
            width: '100%', marginTop: 8, padding: '14px 16px', borderRadius: 14,
            border: `1.5px solid ${BRAND.hairline}`, fontSize: 16, fontWeight: 600,
            color: BRAND.ink, fontFamily: 'Prompt', outline: 'none', background: BRAND.paper,
          }} />

        <button onClick={submit} disabled={!ready} style={{
          marginTop: 20, width: '100%', background: ready ? BRAND.navy : BRAND.muted,
          color: '#fff', border: 'none', borderRadius: 14, padding: '15px',
          fontFamily: 'Prompt', fontSize: 16, fontWeight: 700,
...
function HomeScreen({ user, style, onPickGame, onLeaderboard, onProfile }) {
  ...
  React.useEffect(() => {
    let alive = true;
    if (window.ArcadeAPI) {
      window.ArcadeAPI.loadScores().then(s => { if (alive) setScores(s || []); });
    } else { setScores([]); }
    return () => { alive = false; };
  }, []);
  const mine = (scores || []).filter(s => (user.empId && s.empId === user.empId) || s.name === user.name);
  const myPlays = mine.length;
  const distinct = new Set(mine.map(s => s.gameId)).size;
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {/* Quick stat strip — my own activity (real) */}
      <div style={{ padding: '0 22px 12px', background: BRAND.paper, display: 'flex', gap: 10 }}>
        <QuickStat label="เล่นของฉัน" value={scores === null ? '…' : String(myPlays)} sub="ครั้ง" color={BRAND.navy} />
        <QuickStat label="เกมที่ลอง" value={scores === null ? '…' : `${distinct}/${GAMES.length}`} sub="เกม" color={BRAND.teal} />
      </div>
      ...
      <div style={{
        ...
        <div style={{ marginTop: 22, marginBottom: 8, fontSize: 13, fontWeight: 600, color: BRAND.inkSoft }}>คะแนนสูงสุด</div>
        <TopScoreStrip scores={scores} />
      </div>
...
function TopScoreStrip({ scores }) {
  if (scores === null) {
    return <div style={{ fontSize: 12, color: BRAND.muted, padding: '6px 2px' }}>กำลังโหลด…</div>;
  }
  ...
  if (!hasAny) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: 14, fontWeight: 700, color: BRAND.ink }}>ยังไม่มีคะแนน</div>
        <div style={{ fontSize: 12, color: BRAND.inkSoft, marginTop: 3 }}>เล่นเกมแรกแล้วชื่อคุณจะมาอยู่ตรงนี้ ✨</div>
      </div>
    );
  }
  return (
    <div style={{
      ...
      {GAMES.map(g => {
        const top = window.ArcadeAPI ? window.ArcadeAPI.rankScores(scores, g.id)[0] : null;
        return (
          <div key={g.id} style={{
            ...
            {top ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: g.color, fontWeight: 700 }}>{fmt.num(top.score)} {top.unit}</div>
                </div>
              </div>
            ) : (
              <div style={{ fontSize: 12, color: BRAND.muted, marginTop: 12 }}>ยังไม่มีผู้เล่น</div>
            )}
          </div>
        );
      })}
...
function LeaderboardScreen({ user, onBack }) {
  ...
  React.useEffect(() => {
    let alive = true;
    if (window.ArcadeAPI) window.ArcadeAPI.loadScores().then(s => { if (alive) setScores(s || []); });
    else setScores([]);
    return () => { alive = false; };
  }, []);

  const ranked = (scores && window.ArcadeAPI) ? window.ArcadeAPI.rankScores(scores, gameId) : [];
  const isMine = (r) => (user.empId && r.id === user.empId) || r.name === user.name;
  const myIdx = ranked.findIndex(isMine);
  const myRank = myIdx >= 0 ? myIdx + 1 : null;

  let display;
  if (mode === 'global') {
    display = ranked.slice(0, 10).map((r, i) => ({ ...r, rank: i + 1, isMe: isMine(r) }));
  } else if (myIdx < 0) {
    display = [];
  } else {
    const start = Math.max(0, myIdx - 2);
    display = ranked.slice(start, myIdx + 3).map((r, i) => ({ ...r, rank: start + i + 1, isMe: isMine(r) }));
  }
  const loading = scores === null;
  const empty = !loading && ranked.length === 0;
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: BRAND.paper, overflow: 'hidden' }}>
      ...
      {/* podium for global */}
      {mode === 'global' && ranked.length > 0 && (
        <div style={{ padding: '4px 22px 8px', display: 'flex', alignItems: 'flex-end', gap: 8, justifyContent: 'center' }}>
          {ranked[1] && <PodiumCard rank={2} row={ranked[1]} color={game.color} />}
          {ranked[0] && <PodiumCard rank={1} row={ranked[0]} color={game.color} big />}
          {ranked[2] && <PodiumCard rank={3} row={ranked[2]} color={game.color} />}
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 14px 24px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', color: BRAND.muted, fontSize: 13, padding: '30px 0' }}>กำลังโหลด…</div>
        ) : empty ? (
          <EmptyBoard game={game} />
        ) : (mode === 'me' && myRank === null) ? (
          <div style={{ textAlign: 'center', color: BRAND.inkSoft, fontSize: 13, padding: '30px 16px' }}>
            คุณยังไม่มีคะแนนในเกมนี้ — ลองเล่นดูสิ!
          </div>
        ) : (
          <div style={{
            ...
            {display.map((r, i) => (
              <LeaderRow key={i} rank={r.rank} row={r} color={game.color} isMe={r.isMe} />
            ))}
          </div>
        )}
      </div>
...
function EmptyBoard({ game }) {
  return (
    <div style={{
      background: '#fff', borderRadius: 20, padding: '34px 20px', textAlign: 'center',
      boxShadow: '0 2px 8px rgba(0,0,0,0.04)',
    }}>
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <GameIcon kind={game.icon} size={52} color="#fff" bg={game.color} />
      </div>
      <div style={{ fontSize: 15, fontWeight: 700, color: BRAND.ink, marginTop: 12 }}>ยังไม่มีใครเล่น {game.name}</div>
      <div style={{ fontSize: 12, color: BRAND.inkSoft, marginTop: 4 }}>เป็นคนแรกที่ขึ้นกระดานผู้นำกันเลย!</div>
    </div>
  );
}
...
function ProfileScreen({ user, onBack, onLogout, onAdmin }) {
  ...
  const distinct = new Set(mine.map(s => s.gameId)).size;
  let bestRank = null;
  if (window.ArcadeAPI && scores) {
    GAMES.forEach(g => {
      const idx = window.ArcadeAPI.rankScores(scores, g.id).findIndex(isMine);
      if (idx >= 0) bestRank = bestRank === null ? idx + 1 : Math.min(bestRank, idx + 1);
    });
  }
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: BRAND.paper, overflow: 'auto' }}>
      ...
      <div style={{ padding: '0 22px', display: 'flex', flexDirection: 'column', gap: 12, paddingBottom: 32 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          <Kpi label="เล่นทั้งหมด" value={loading ? '…' : String(mine.length)} color={BRAND.navy} />
          <Kpi label="เกมที่ลอง" value={loading ? '…' : `${distinct}/${GAMES.length}`} color={BRAND.teal} />
          <Kpi label="อันดับดีสุด" value={loading ? '…' : (bestRank ? ('#' + bestRank) : '—')} color={BRAND.red} />
        </div>

        <div style={{ marginTop: 4 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: BRAND.inkSoft, marginBottom: 8 }}>คะแนนของคุณในแต่ละเกม</div>
          {(!loading && mine.length === 0) ? (
            <div style={{ background: '#fff', borderRadius: 16, padding: '24px 16px', textAlign: 'center', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: BRAND.ink }}>ยังไม่มีสถิติ</div>
              <div style={{ fontSize: 12, color: BRAND.inkSoft, marginTop: 3 }}>เริ่มเล่นเกมแรกเพื่อสะสมคะแนนกันเลย!</div>
            </div>
          ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {GAMES.map((g) => {
              const ranked = window.ArcadeAPI ? window.ArcadeAPI.rankScores(scores || [], g.id) : [];
              const myEntry = ranked.find(isMine);
              const topScore = ranked[0] ? ranked[0].score : null;
              const lowerBetter = g.id === 'memory' || g.id === 'reaction';
              let pct = 0;
              if (myEntry && topScore) {
                pct = lowerBetter
                  ? Math.max(15, Math.min(100, (topScore / myEntry.score) * 100))
                  : Math.max(15, Math.min(100, (myEntry.score / topScore) * 100));
              }
              return (
                <div key={g.id} style={{
                  background: '#fff', borderRadius: 16, padding: '10px 12px',
                  display: 'flex', alignItems: 'center', gap: 12,
                  boxShadow: '0 1px 4px rgba(0,0,0,0.04)', opacity: myEntry ? 1 : 0.6,
                }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
                      ...
                      {myEntry ? (
                        <div style={{ fontSize: 13, fontWeight: 700, color: g.color }}>{fmt.num(myEntry.score)} <span style={{ fontSize: 10, color: BRAND.muted }}>{myEntry.unit}</span></div>
                      ) : (
                        <div style={{ fontSize: 11, color: BRAND.muted }}>ยังไม่เล่น</div>
                      )}
                    </div>
